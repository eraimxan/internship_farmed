...
session.headers.update({
    ...
    # ⚠️ Обновляй куки перед каждым запуском!
...
# ─────────────────────────────────────────────────────────────────────
# ОПРЕДЕЛЕНИЕ КОЛОНОК С ДАННЫМИ (годы)
# Сайт возвращает колонки вида: "y122023" (сумма) и "122023" (кол-во)
# Пустая строка "" = данных нет за этот год
# Число = данные есть
# ─────────────────────────────────────────────────────────────────────

def clean_row(row):
    """
    Преобразует значения колонок с данными:
    - ""  (пустая строка) → None  (данных нет на сайте)
    - "0" → 0
    - числа → int/float
    ...
    for k, v in row.items():
        # Колонки с данными: начинаются с цифры или "y" + цифра
        is_data_col = (
        ...
        if is_data_col:
            if v == "" or v is None:
                cleaned[k] = None          # ← данных нет на сайте
            else:
                try:
                    cleaned[k] = int(v)
...
def fetch(p_terms, parent_id="", retries=5, delay=1.5, is_root=False):
    """
    is_root=True — это запрос первого уровня (depth=0).
    Если он вернул пустоту — куки точно устарели, останавливаемся.
    """
    data = {
    ...
    for attempt in range(retries):
        try:
            ...
            if len(text) <= 2 or text in ("[]", "null", ""):
                ...
                # Корневой узел пустой = куки мертвы, дальше смысла нет
                if is_root:
                    raise CookieExpiredError(
                        "Корневой запрос вернул пустоту — куки устарели!"
                    )
...
def fetch_recursive(p_terms, parent_id="", depth=0, parent_text="", max_depth=10):
    ...
    # Только первый вызов (depth=0, parent_id="") помечаем как root
    is_root = (depth == 0 and parent_id == "")
    rows = fetch(p_terms, parent_id, is_root=is_root)
    all_rows = []

    for row in rows:
        enriched = clean_row(dict(row))   # ← очищаем значения
...
PROGRESS_FILE = os.path.join(OUTPUT_DIR, "all_progress.csv")
...
try:
    ...
    for oblast_num, (oblast_name, districts) in enumerate(ALL_REGIONS.items(), 1):
        log(f"\n{'='*60}")
        log(f"[{oblast_num}/{total_regions}] {oblast_name}  ({len(districts)} районов)")
        log(f"{'='*60}")
    ...
    log(f"  Следующий район: см. done.txt — продолжит с места остановки.")
    log(f"{'!'*60}")
...
final_path = os.path.join(OUTPUT_DIR, "ALL_KAZAKHSTAN_FINAL.csv")
