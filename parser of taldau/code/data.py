...
session.headers.update({
    ...
    # ⚠️ ОБНОВИ КУКИ если перестанет работать (F12 → Network → любой запрос → Cookie)
    "Cookie": "_ym_uid=1758390801129425250; _ym_d=1758390801; ASP.NET_SessionId=kk31rgemdjl30b0szyeofjjv"
})
...
print(f"Областей/городов: {len(ALL_REGIONS)}")
print(f"Районов всего:    {total_districts}")
print()
...
def fetch(p_terms, parent_id="", retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"  ⚠️ Таймаут (попытка {attempt+1}/{retries}), жду 5 сек...")
            time.sleep(5)
...
# Файл прогресса — чтобы продолжить с места обрыва
PROGRESS_FILE = os.path.join(OUTPUT_DIR, "all_progress.csv")
DONE_FILE     = os.path.join(OUTPUT_DIR, "done.txt")

# Загружаем уже обработанные районы (если скрипт запускался раньше)
done_districts = set()
if os.path.exists(DONE_FILE):
    with open(DONE_FILE, "r", encoding="utf-8") as f:
        done_districts = set(line.strip() for line in f if line.strip())
    print(f"▶ Продолжаем с места остановки. Уже готово: {len(done_districts)} районов\n")

grand_total = []
# Загружаем уже накопленные данные
if os.path.exists(PROGRESS_FILE) and done_districts:
    ...
    print(f"  Загружено {len(grand_total)} строк из предыдущего запуска\n")

oblast_num = 0
for oblast_name, districts in ALL_REGIONS.items():
    oblast_num += 1
    print(f"\n{'='*60}")
    print(f"[{oblast_num}/{len(ALL_REGIONS)}] {oblast_name}  ({len(districts)} районов)")
    print(f"{'='*60}")
    ...
    for i, item in enumerate(districts, 1):
        ...
        if district_key in done_districts:
            print(f"  [{i}/{len(districts)}] {item['name']} — пропущен (уже есть)")
            continue

        print(f"  [{i}/{len(districts)}] {item['name']}...", end=" ", flush=True)
    ...
    if oblast_data:
        ...
        print(f"  💾 {oblast_path}  ({len(oblast_data)} строк)")
...
print(f"\n{'='*60}")
print(f" ГОТОВО!")
print(f"   Областей:  {final_df['oblast'].nunique()}")
print(f"   Районов:   {final_df['region'].nunique()}")
