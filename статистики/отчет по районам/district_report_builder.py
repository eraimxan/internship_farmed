...
# SELECT column positions (0-based):
...
def write_section(ws, start_row, agg, total, col, font_main, font_small):
    ...
    for fgr in sorted(agg.keys(), key=lambda x: (x is None, x)):
        ...
        for l1n in sorted(d["lvl1"].keys()):
            l1d = d["lvl1"][l1n]
            ws.cell(row=row, column=7, value=l1n).font = Font(name='Times New Roman', size=8, italic=True)
...
def create_table_in_excel(db_params, file_path, kato_pattern, sheet_name, datereport=None):
    """
    Called by district runner scripts.  Opens its own connection, finds the
    latest table and date automatically, writes one sheet to file_path.
    """
    try:
        ...
        wb.save(file_path)
        print(f"✅ {sheet_name} → {file_path} ({len(data)} строк, таблица={table}, дата={datereport})")

    except Exception as e:
        import traceback
        print(f"❌ Ошибка при обработке {sheet_name}: {e}")
        traceback.print_exc()
