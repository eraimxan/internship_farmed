"""
cpi_scripts/config.py — Configuration for CPI (Consumer Price Index) parser.

Source: https://taldau.stat.gov.kz/ru/NewIndex/GetIndex/703076?keyword=инфляция
Index ID: 703076 — Индексы потребительских цен
"""
...
DATA_CSV        = OUTPUT_DIR / "cpi_data.csv"
CHECKPOINT_CSV  = OUTPUT_DIR / "cpi_checkpoint.csv"   # partial progress saved here
EXCEL_OUT       = OUTPUT_DIR / "CPI_Kazakhstan.xlsx"
HTML_OUT        = OUTPUT_DIR / "cpi_kazakhstan.html"

# Cookie file (try the parent project's cookie, or set COOKIE_STRING directly)
COOKIE_FILE = HERE.parent / "data" / "cookie.txt"
COOKIE_STRING = ""   # override here or leave empty to read from file
...
RETRY_COUNT          = 5
REQUEST_TIMEOUT      = 90
RETRY_BACKOFF_BASE   = 15     # seconds to wait after a timeout/connection error (multiplied by attempt)
MAX_DEPTH            = 7
FETCH_WORKERS        = 2      # parallel region workers (2 is safe; 4 triggers rate limiting)

# ─────────────────────────────────────────────
# TARGET COMPARISON PERIODS
# These are the two period comparison types we want to extract.
# The script will match them by name (case-insensitive substring).
# ─────────────────────────────────────────────
TARGET_PERIODS = {
    "yoy":         "к соответствующему периоду прошлого года",
    "prev_period": "к предыдущему периоду",
}

# Human-readable labels for Excel/HTML headers
PERIOD_LABELS = {
    "yoy":         "К соотв. периоду прошлого года",
    "prev_period": "К предыдущему периоду",
}

# ─────────────────────────────────────────────
# KNOWN API PARAMETERS FOR INDEX 703076
# Discovered from GetPeriodList + GetSegmentList + region tree.
# These are hardcoded to avoid repeated discovery calls and ensure
# correctness. They can be overridden in fetch.py --params-override.
# ─────────────────────────────────────────────
KNOWN_PARAMS = {
    # Period type: "Квартал" (quarterly, period_id=5)
    "period_id":  "5",

    # Measure ID: 7 (confirmed from live network capture)
    "measure_id": "7",

    # DicIds: 67=Регионы(КАТО), 848=Соотнесение периодов, 2753=Товары и услуги
    "dic_ids": "67,848,2753",

    # Goods dimension: hidden root=7358, actual root=4772381 ("Товары и услуги")
    "goods_class_root": "7358",
    "goods_root":       "4772381",

    # Comparison type term IDs
    "comparison_yoy":         "2695732",  # к соответствующему периоду прошлого года
    "comparison_prev_period": "2695730",  # к предыдущему периоду
}

# ─────────────────────────────────────────────
# CPI REGIONS — oblast-level IDs (CPI data is only available at oblast level)
# Discovered from GetIndexTreeData region tree for index 703076.
# ─────────────────────────────────────────────
CPI_REGIONS: list[dict] = [
    {"id": "741880",  "name": "РЕСПУБЛИКА КАЗАХСТАН"},
    {"id": "247783",  "name": "АКМОЛИНСКАЯ ОБЛАСТЬ"},
    {"id": "248875",  "name": "АКТЮБИНСКАЯ ОБЛАСТЬ"},
    {"id": "250502",  "name": "АЛМАТИНСКАЯ ОБЛАСТЬ"},
    {"id": "252311",  "name": "АТЫРАУСКАЯ ОБЛАСТЬ"},
    {"id": "253160",  "name": "ЗАПАДНО-КАЗАХСТАНСКАЯ ОБЛАСТЬ"},
    {"id": "255577",  "name": "ЖАМБЫЛСКАЯ ОБЛАСТЬ"},
    {"id": "256619",  "name": "КАРАГАНДИНСКАЯ ОБЛАСТЬ"},
    {"id": "258742",  "name": "КОСТАНАЙСКАЯ ОБЛАСТЬ"},
    {"id": "260099",  "name": "КЫЗЫЛОРДИНСКАЯ ОБЛАСТЬ"},
    {"id": "260907",  "name": "МАНГИСТАУСКАЯ ОБЛАСТЬ"},
    {"id": "261475",  "name": "ЮЖНО-КАЗАХСТАНСКАЯ ОБЛАСТЬ"},
    {"id": "263009",  "name": "ПАВЛОДАРСКАЯ ОБЛАСТЬ"},
    {"id": "264023",  "name": "СЕВЕРО-КАЗАХСТАНСКАЯ ОБЛАСТЬ"},
    {"id": "264990",  "name": "ВОСТОЧНО-КАЗАХСТАНСКАЯ ОБЛАСТЬ"},
    {"id": "268012",  "name": "Г.АСТАНА"},
    {"id": "268020",  "name": "Г.АЛМАТЫ"},
    {"id": "20242100","name": "Г.ШЫМКЕНТ"},
    {"id": "20243032","name": "ТУРКЕСТАНСКАЯ ОБЛАСТЬ"},
    {"id": "77208139","name": "ОБЛАСТЬ ЖЕТІСУ"},
    {"id": "77208140","name": "ОБЛАСТЬ ҰЛЫТАУ"},
    {"id": "77208141","name": "ОБЛАСТЬ АБАЙ"},
]

# ─────────────────────────────────────────────
# EXCEL LEVEL COLORS  (depth → bg, fg, bold, size)
# ─────────────────────────────────────────────
LEVEL_COLORS = {
...
# ─────────────────────────────────────────────
# REGIONS — all districts and cities of Kazakhstan
# IDs from taldau.stat.gov.kz
# ─────────────────────────────────────────────
